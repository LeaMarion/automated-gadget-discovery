...
class SimDistance():
    ...
    def distance_matrix_population(self):
        ...
        for i, j in zip(np.triu_indices(len(self.patterns),1)[0], np.triu_indices(len(self.patterns),1)[1]):
            ...
            np.save(self.results_folder+self.supplementary_folder+'distance_matrix_{}_run_{}.npy'.format(self.sim_measure, self.run), self.pattern_distance_matrix)
    ...
    def sim_measure_init_sets(self, pattern_i, pattern_j):
        ...
        #save d matrix.
        np.save(self.results_folder+self.supplementary_folder+'d_matrix_patt_{}_{}_run_{}.npy'.format(pattern_i.id,pattern_j.id, self.run), d)
    ...
    def init_set(self, pattern):
        ...
        init_set = []
        dataset_size = int(self.max_initset_size / pattern.F)
        
        #get raw data
        if self.results_folder:
            raw_data = np.load(self.results_folder + 'data_cycle_'+str(pattern.cycle[0])+'_run_'+str(self.run)+'.npy') #sequences with environment notation.
            np.random.shuffle(raw_data)
        else:
            raw_data = []
            warnings.warn('No folder was given to the exchanger. The exchangeability cannot be computed.')
        
        #get initialization sequences
        for sequence in raw_data[:dataset_size]:
        ...
        np.save(self.results_folder+self.supplementary_folder+'init_set_pattern_{}_run_{}.npy'.format(pattern.id, self.run), np.asarray(init_set, dtype=object))
...
if __name__ == "__main__":
    ...
    pattern_manager = PatternManager(features_removed=0)

    for ag in range(NUM_AGENTS):
        rule_output = np.load(results_folder + 'sequence_mining/' + 'final_rules_cycle_0_run_' + str(ag) + '.npy',
                              allow_pickle=True).item()

        patterns = pattern_manager.pattern_processing(list(rule_output.values()))

    np.save(results_folder + 'pattern_dict.npy', pattern_manager.pattern_dict)
    ...
    processor_file_path = results_folder + 'pos_reward_data_cycle_0_run_0.npy'#just to compute the characteristics of the sequences.
    seq_processor = SeqProcessor(file_path=processor_file_path)
